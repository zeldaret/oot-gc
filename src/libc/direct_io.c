...
size_t fwrite(const void* buffer, size_t memb_size, size_t num_memb, FILE* file) {
    ...
    buff = (!file->mode.binary_io || (int)file->mode.file_kind == __unavailable_file || file->mode.buffer_mode == 2 ||
            file->mode.buffer_mode == 1);
    ...
    if (rem_bytes && (file->buffer_ptr != file->buffer || buff)) {
        ...
        do {
            ...
            if (file->buffer_len == 0 && (int)file->mode.file_kind == __unavailable_file) {
