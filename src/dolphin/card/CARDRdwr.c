...
static void BlockReadCallback(s32 channel, s32 result) {
    ...
    card->buffer = (void*)((u8*)card->buffer + CARD_SEG_SIZE);
    if (--card->repeat <= 0) {
        goto error;
    }
...
static void BlockWriteCallback(s32 channel, s32 result) {
    ...
    card->buffer = (void*)((u8*)card->buffer + CARD_PAGE_SIZE);
    if (--card->repeat <= 0) {
        goto error;
    }
...
s32 CARDGetXferredBytes(s32 chan) { return __CARDBlock[chan].xferred; }
